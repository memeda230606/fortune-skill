...
3. **LLM 负责解释**：LLM 负责解释、综合、取舍和写作，不得用自由推理替代可计算结果。
4. **双重验证**：八字和紫微两套系统交叉印证。
5. **经典依据**：分析需结合传统典籍论述；深度报告必须尽量用足 `references/classical-texts.md` 与 `references/classical-rules.json`，把调候、格局、病药、通关、作用优先级等规则转化为明确判断依据。
6. **现代应用**：将传统命理转化为现代生活建议。
7. **免责声明**：明确说明仅供参考，不构成决策依据。
